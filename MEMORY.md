...
- [fact] (trading) Bot crash storm: 6 consecutive restart failures (missing yaml module + API proxy down) wiped bot_state.json to empty, leaving 4 positions (RESOLV, AERO, JTO, HYUNDAI) unmanaged with zero SL/TP protection. — distilled topics
- [decision] (trading) Added `_reconcile_state_with_exchange()` to bot.py — on startup, fetches exchange positions, adopts orphans, removes phantoms, validates matches. Exchange is source of truth. Wrapped in try/except so it never crashes startup. — distilled topics
- [decision] (trading) Added `track_manual_positions: true` to config.yml as belt-and-suspenders fix because `get_positions()` API returned inconsistent results (empty during reconciliation, populated during ticks). Ensures bot adopts all positions regardless of state file. — distilled topics
- [pattern] (trading) Added immediate `_save_state()` after each position open to prevent state loss if crash occurs between position open and next periodic save. — distilled topics
- [fact] (trading) DSL high-water marks reset to defaults (high_water_roe=0, no tier locks) on position adoption/reconciliation. Can't reconstruct historical high-water from exchange data alone — accepted tradeoff. — distilled topics

## Session Extract — 2026-03-24 [auto]

- [fact] (system) Nightly cleanup cron job references `scripts/memory-nightly-cleanup.sh` but the actual script is at `scripts/memory/memory-nightly-cleanup.sh`. The path mismatch needs correction to ensure reliable execution. — distilled topics
