...
```
ai_trader.py writes → /root/.openclaw/workspace/lighter-bot/ai-decision.json
bot.py reads → consumes decision in _tick()
bot.py writes → /root/.openclaw/workspace/lighter-bot/ai-result.json (outcome)
...
class LighterCopilot:
    def __init__(self, cfg):
        # ... existing ...
        self._ai_decision_file = "/root/.openclaw/workspace/lighter-bot/ai-decision.json"
        self._ai_result_file = "/root/.openclaw/workspace/lighter-bot/ai-result.json"
...
Type=simple
User=root
WorkingDirectory=/root/.openclaw/workspace/lighter-bot/ai-trader
...
*/5 * * * * /root/.openclaw/workspace/lighter-bot/ai-trader/healthcheck.sh

# SQLite backup every 6 hours
0 */6 * * * cp /root/.openclaw/workspace/lighter-bot/ai-trader/state/trader.db \
              /root/.openclaw/workspace/lighter-bot/ai-trader/state/trader.db.bak

# Reflection agent every 4 hours
0 */4 * * * /usr/bin/python3 /root/.openclaw/workspace/lighter-bot/ai-trader/reflection.py
```
...
```json
{
  ...
  "signals_file": "../lighter-bot/signals.json",
...
2. Read `lighter-copilot/bot.py` — understand current signal processing and position management
3. Read `lighter-bot/scripts/opportunity-scanner.ts` — understand signals.json format
