...
After completing a research task, log the execution:
```bash
bun skills/skill-monitor/scripts/log-run.ts deep-research-pro <success|failure|partial> \
  --task "brief description of the research topic" --duration <ms>
```
If the user provides feedback (too verbose, missed key source, etc.), add `--feedback "their feedback"`.
